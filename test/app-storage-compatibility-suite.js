function testAppStorageDocumentCompatibility(extendedContext) {
  ...
  suite('storage key change', function() {
    var storage;
    setup(function() {
      storage = fixture('BasicStorage');
      return storage.transactionsComplete;
    });
    test('reinitializes stored value regardless of previous initialization success', function(done) {
      storage.initializeStoredValue = sinon.stub().returns(Promise.reject());
      storage.set('key', 'other-key');
      expect(storage.initializeStoredValue.calledOnce).to.be.true;
      setTimeout(function() {
        storage.set('key', 'another-other-key');
        expect(storage.initializeStoredValue.calledTwice).to.be.true;
        done();
      }, 1);
    });
  });
  ...
  suite('app storage compatibility: <' + context.tagName + '>', function() {
    suite('basic storage scenarios', function() {
      ...
      test('knows how to store undefined', function() {
        storage.set('data', undefined);
        return storage.transactionsComplete.then(function() {
          // regression test - used to reject before the fix
